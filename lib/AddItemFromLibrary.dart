...
class AddItemFromLibrary extends StatefulWidget {
  final int gameId;
  ...
  @override
  _AddItemFromLibraryState createState() => _AddItemFromLibraryState();
}

class _AddItemFromLibraryState extends State<AddItemFromLibrary>
      ...
      if (value.success) {
        ...
        for (var i = 0; i < items.length; i++) {
          for (var j = 0; j < itemArr.length; j++) {
            if (items[i].name == itemArr[j].itemname) {
        ...
        for (var i = 0; i < itemArr.length; i++) {
          ...
          for (var j = 0; j < items.length; j++) {
            if (itemArr[i].itemname == items[j].name) {
              f = 1;
              break;
            }
          }
...
// for search list Api call
      ...
      if (value.success) {
        ...
        for (var i = 0; i < items.length; i++) {
          for (var j = 0; j < libraryItemList.length; j++) {
            if (items[i].name == libraryItemList[j].item_name) {
        ...
        for (var i = 0; i < libraryItemList.length; i++) {
          var f = 0;
          for (var j = 0; j < items.length; j++) {
            if (libraryItemList[i].item_name == items[j].name) {
              f = 1;
              break;
            }
          }
  ...
  void _onNextPress() async {
    ...
    for (var i = 0; i < libraryItemListData.length; i++) {
      if (libraryItemListData[i].itemId == libraryItemListData[i].itemId) {}
    ...
    if (maxHuntItems != null &&
        mainArrData.length + widget.itemLength > maxHuntItems) {
      ...
      if (mainArrData.isNotEmpty) {
        ...
        ApiService.addGameItemLibrary(payload).then((respoData) {
